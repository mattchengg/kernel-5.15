...
static int tex_boosted_fit_cpus(struct tp_env *env)
{
	...
	if (env->cpu_sync && cpumask_test_cpu(this_cpu, &env->cpus_allowed)) {
...
{
	int group_idx;
	struct cpumask *override;

	group_idx = cpuctl_task_group_idx(p);

	override = override_ta_binding_mask(p);
	if (override != NULL)
		return override;
...
static int get_pl_init_index(struct tp_env *env)
{
	...
	/* When sched_io_boost or sched_boost, start from MID-HF */
	if ((emstune_sched_io_boost() || emstune_sched_boost())
...
static int normal_fit_cpus(struct tp_env *env)
{
	...
	/*
	 * If WF_CURRENT_CPU flag is set, scheduler should select the wakee
	 * to current cpu.
	 */
	if (env->cpu_sync && cpumask_test_cpu(this_cpu, &env->cpus_allowed)) {
			   ...
			   int sd_flag, int wake_flag)
{
	struct tp_env env = {
		...
		.sync = (wake_flag & WF_SYNC) &&
			!(current->flags & PF_EXITING),
		.cpu_sync = (wake_flag & WF_CURRENT_CPU),
		.prev_cpu = task_cpu(p),
